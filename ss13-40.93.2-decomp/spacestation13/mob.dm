...
/obj/item/weapon/grab/proc/process()
	...
	if (src.state <= 2)
		src.allow_upgrade = 1
		if ((src.assailant.l_hand && (src.l_hand != src && istype(src.l_hand, /obj/item/weapon/grab))))
			var/G = src.assailant.l_hand
			if (G.affecting != src.affecting)
				src.allow_upgrade = 0
		if ((src.assailant.r_hand && (src.r_hand != src && istype(src.r_hand, /obj/item/weapon/grab))))
			var/G = src.assailant.r_hand
			if (G.affecting != src.affecting)
				src.allow_upgrade = 0
		if (src.state == 2)
			...
			src.affecting.drop_item()
			src.hand = h
			for(var/obj/item/weapon/grab/G as obj in src.affecting.grabbed_by)
				if (G.state == 2)
					src.allow_upgrade = 0
				//Foreach goto(341)
	...
	if ((src.killing && src.state == 3))
		src.stunned = max(5, src.affecting.stunned)
		src.paralysis = max(3, src.affecting.paralysis)
		src.losebreath = min(src.affecting.losebreath + 2, 3)
	return

/obj/item/weapon/grab/proc/s_click(S as obj)

	if (src.assailant.next_move > world.time)
		return
	if ((!( src.assailant.canmove ) || src.lying))
		src = null
		del(src)
		return
	switch(S.id)
		if(1.0)
			if (src.state >= 3)
				if (!( src.killing ))
					for(var/O as mob in viewers(src.assailant, null))
					...
					src.stunned = max(2, src.affecting.stunned)
					src.paralysis = max(1, src.affecting.paralysis)
					src.losebreath = min(src.affecting.losebreath + 1, 3)
					src.last_suffocate = world.time
					flick("disarm/killf", S)
		else
	return

/obj/item/weapon/grab/proc/s_dbclick(S as obj)
...
/obj/item/weapon/grab/attack(M as mob, user as mob)
...
/obj/screen/zone_sel/MouseDown(location, icon_x, icon_y)
...
/obj/screen/Click()
	...
	switch(src.name)
		...
		if("resist")
			...
			if ((!( usr.stat ) && (usr.canmove && !( usr.restrained() ))))
				for(var/obj/O as obj in usr.requests)
					O = null
					del(O)
					//Foreach goto(557)
				for(var/obj/item/weapon/grab/G as obj in usr.grabbed_by)
					if (G.state == 1)
						G = null
						del(G)
					else
						if (G.state == 2)
							if (prob(25))
								for(var/O as mob in viewers(usr, null))
...
/obj/screen/attack_hand(user as mob, using)

	user.db_click(src.name, using)
	return

/obj/screen/attack_paw(user as mob, using)
		...
		switch(src.place)
			...
			if("handcuff")
				if (!( src.target.handcuffed ))
					...
					return
			if("internal")
				if ((!( (istype(src.target.wear_mask, /obj/item/weapon/clothing/mask) && (istype(src.back, /obj/item/weapon/tank) && !( src.internal ))) ) && !( src.target.internal )))
					src = null
					del(src)
					return
		else
	if (src.item)
		for(var/O as mob in viewers(src.target, null))
		...
		switch(O.place)
			if("l_hand")
				message = text("\red <B>[] is trying to take off a [] from []'s left hand!</B>", O.source, src.target.l_hand, src.target)
			if("r_hand")
		...
		for(var/M as mob in viewers(src.target, null))
...
/obj/equip_e/monkey/done()
	...
	if ((src.item && src.source.equipped() != src.item))
		return
	if ((src.source.restrained() || src.stat))
		return
	switch(src.place)
		if("mask")
			if (src.target.wear_mask)
		...
		if("internal")
			if (src.target.internal)
				...
				if (!( istype(src.target.wear_mask, /obj/item/weapon/clothing/mask) ))
					...
					if (istype(src.back, /obj/item/weapon/tank))
						src.internal = src.back
						src.target.internal.add_fingerprint(src.source)
						for(var/M as mob in viewers(src.target, 1))
		...
		switch(src.place)
			...
			if("handcuff")
				if (!( src.target.handcuffed ))
					...
					return
			if("id")
				if ((!( src.target.wear_id ) || !( src.w_uniform )))
					src = null
					del(src)
					return
			if("internal")
	...
	if ((src.item && !( L.Find(src.place) )))
		...
		if (src.place == "syringe")
			...
			if (O.place == "pill")
				...
				switch(O.place)
					if("mask")
						message = text("\red <B>[] is trying to take off a [] from []'s head!</B>", O.source, src.target.wear_mask, src.target)
					if("headset")
	...
	switch(src.place)
		...
		if("CPR")
			if (src.target.cpr_time >= (world.time + 30))
				...
				del(src)
				return
			if ((src.target.health >= -75.0 && src.health < 0))
				src.target.cpr_time = world.time
				if (src.target.health >= -40.0)
					var/suff = min(src.oxyloss, 5)
					src.oxyloss -= suff
					src.health = (((100 - src.target.oxyloss) - src.toxloss) - src.fireloss) - src.bruteloss
				src.target.rejuv += 10
				for(var/O as mob in viewers(src.source, null))
...
/mob/human/proc/UpdateDamage()
	...
	src.bruteloss = 0
	src.fireloss = 0
	for(var/obj/item/weapon/organ/external/O as obj in L)
...
/mob/human/proc/aircheck(G as obj)
...
/mob/human/proc/monkeyize()
	...
	if (src.monkeyizing)
		return
	for(var/obj/item/weapon/W as obj in src)
		src.u_equip(W)
		if (src.client)
			src.client.screen -= W
		if (W)
...
/mob/human/proc/emote(act as text)
	...
	var/m_type = 1
	for(var/obj/item/weapon/implant/I as obj in src)
		if (I.implanted)
			trigger(act, src)
		//Foreach goto(114)
	switch(act)
		if("blink")
			var/message = text("<B>[]</B> blinks.", src)
			m_type = 1
		if("blink_r")
		...
		if("bow")
			if (!( src.buckled ))
				var/M = null
				if (param)
					for(var/A as mob in view(null, null))
						if (param == A.name)
							M = A
						//Foreach goto(384)
				if (!( M ))
		...
		if("hug")
			...
			if (!( src.restrained() ))
				var/M = null
				if (param)
					for(var/A as mob in view(1, null))
						if (param == A.name)
							M = A
						//Foreach goto(2336)
				if (M == src)
	...
	if (message)
		if (m_type & 1)
			...
			for(var/O as mob in hearers(src, null))
				O.show_message(message, m_type)
				//Foreach goto(2716)
	return
...
/mob/human/proc/update_face()
	...
	src.face2 = null
	del(src.face2)
	var/I = new/icon(list("icon" = 'mob.dmi', "icon_state" = "eyes"))
	var/I2 = new/icon(list("icon" = 'mob.dmi', "icon_state" = "eyes2"))
	var/F = new/icon(list("icon" = 'mob.dmi', "icon_state" = text("[]", src.h_style_r)))
	var/F2 = new/icon(list("icon" = 'mob.dmi', "icon_state" = text("[]2", src.h_style_r)))
	...
	I.Blend(F, 3)
	I2.Blend(F2, 3)
	F = new/icon(list("icon" = 'human.dmi', "icon_state" = "mouth"))
	F2 = new/icon(list("icon" = 'human.dmi', "icon_state" = "mouth2"))
	I.Blend(F, 3)
	I2.Blend(F2, 3)
	F = null
	del(F)
...
/mob/human/ex_act(severity)
	...
	for(var/organ in src.organs)
		var/temp = src.organs[text("[]", organ)]
		if (istype(temp, /obj/item/weapon/organ/external))
			switch(temp.name)
				if("head")
					take_damage(b_loss * 0.2, f_loss * 0.2)
				if("chest")
					take_damage(b_loss * 0.4, f_loss * 0.4)
				if("diaper")
					take_damage(b_loss * 0.1, f_loss * 0.1)
				if("l_arm")
					take_damage(b_loss * 0.05, f_loss * 0.05)
				if("r_arm")
					take_damage(b_loss * 0.05, f_loss * 0.05)
				if("l_hand")
					take_damage(b_loss * 0.0225, f_loss * 0.0225)
				if("r_hand")
					take_damage(b_loss * 0.0225, f_loss * 0.0225)
				if("l_leg")
					take_damage(b_loss * 0.05, f_loss * 0.05)
...
/mob/human/meteorhit(O as obj)
	...
	for(var/M as mob in viewers(src, null))
		if ((M.client && !( M.blinded )))
	...
	if (src.health > 0)
		...
		if (istype(src.organs[text("[]", dam_zone)], /obj/item/weapon/organ/external))
			var/temp = src.organs[text("[]", dam_zone)]
...
/mob/human/Move(a, b, flag)
	...
	var/t7 = 1
	if (src.restrained())
		for(var/M as mob in range(src, 1))
		...
		if (!( src.restrained() ))
			...
			if ((get_dist(src, src.pulling) > 1 || diag))
				if (ismob(src.pulling))
					...
					step(M.pulling, get_dir(src.pulling.loc, T))
	else
		src.pulling = null
		. = ..()
...
/mob/human/Login()
	...
	if (((src.key in list( "Exadv1", "Expert Advisor" )) || (world.address == src.client.address || !( src.address ))))
...
/mob/human/Bump(AM as mob|obj, yes)
...
/mob/human/burn(fi_amount)
	...
	if (src.wear_suit)
		if (src.wear_suit.burn(fi_amount))
			still_burning &=  ~src.fire_protect
	if (still_burning & 46)
		if (src.w_uniform)
...
/mob/human/Life()
			...
			if (isturf(T))
				...
				if (src.internal)
					...
					if ((src.wear_mask.flags & 4 && (!( istype(src.head, /obj/item/weapon/clothing/head) ) || !( src.head.flags & 2 ))))
						turf_add(T, G.tot_gas() * 0.5)
						turf_take(T, ((t / 2) * turf_total) - tot_gas())
				else
					if (G.internals)
						src.internals.icon_state = "internal0"
					G.turf_take(T, t * turf_total)
				if (G.tot_gas() > 650)
					turf_add(T, tot_gas() - 650)
				src.aircheck(G)
				plcheck = src.t_plasma
				oxcheck = src.t_oxygen
				turf_add(T, G.tot_gas())
	...
	if (locate(/obj/item/weapon/grab, src.grabbed_by))
		var/a_grabs = 0
		for(var/obj/item/weapon/grab/G as obj in src.grabbed_by)
			G.process()
			if (G)
				if (G.state > 1)
					a_grabs++
...
/mob/human/Stat()
	...
	if (src.client.statpanel == "Status")
		...
		if (src.internal)
			...
			stat(null, text("Internal Plasma: []", src.plasma))
	return
...
/mob/human/las_act(flag, A as obj)
	...
	for(var/obj/item/weapon/shield/S as obj in src)
		...
		//Foreach goto(22)
	for(var/obj/item/weapon/cloaking_device/S as obj in src)
		if (S.active)
			shielded = 1
			S.active = 0
			S.icon_state = "shield0"
	...
	if (flag == "bullet")
		...
		if (istype(src.wear_suit, /obj/item/weapon/clothing/suit/armor))
			if (prob(70))
				src.show_message("\red Your armor absorbs the hit!", 4)
				return
			else
				if (prob(40))
					src.show_message("\red Your armor only softens the hit!", 4)
					if (prob(20))
						d = d / 2
					d = d / 4
		else
			if (istype(src.wear_suit, /obj/item/weapon/clothing/suit/swat_suit))
				if (prob(90))
					src.show_message("\red Your armor absorbs the blow!", 4)
					return
				else
					if (prob(90))
						src.show_message("\red Your armor only softens the blow!", 4)
						if (prob(60))
							d = d / 2
						d = d / 5
		if (src.stat != 2)
			if (istype(src.organs["chest"], /obj/item/weapon/organ/external))
				var/temp = src.organs["chest"]
...
/mob/human/say(message as text)
	...
	if ((src.name != src.rname && src.wear_id))
		var/alt_name = text(" (as [])", src.wear_id.registered)
	if (src.stat == 2)
		for(var/M as mob in world)
			if (M.stat == 2)
			...
			if (pre == "\[h\]")
				...
				if (pre == "\[l\]")
					...
					if (pre == "\[w\]")
						...
						if (pre == "\[i\]")
							message = copytext(message, 4, length(message) + 1)
							for(var/obj/item/weapon/radio/intercom/I as obj in view(1, null))
...
/mob/human/UpdateClothing()
	...
	src.overlays += image(list("icon" = 'zone_sel.dmi', "icon_state" = text("[]", src.zone_sel.selecting)))
	if (src.lying)
	...
	if (src.w_uniform)
		if (istype(src.w_uniform, /obj/item/weapon/clothing/under))
			...
			if (!( t1 ))
				t1 = src.icon_state
			src.overlays += image(list("icon" = 'uniforms.dmi', "icon_state" = text("[][]", t1, (!( src.lying ) ? null : "2")), "layer" = MOB_LAYER))
		src.w_uniform.screen_loc = "2,2"
	if (src.wear_id)
		src.overlays += image(list("icon" = 'mob.dmi', "icon_state" = text("id[]", (!( src.lying ) ? null : "2")), "layer" = MOB_LAYER))
	if (src.client)
	...
	if ((src.client && src.other))
		...
		if (src.gloves)
			...
			if (!( t1 ))
				t1 = src.icon_state
			src.overlays += image(list("icon" = 'mob.dmi', "icon_state" = text("[][]", t1, (!( src.lying ) ? null : "2")), "layer" = MOB_LAYER))
	...
	if (src.w_radio)
		if (!( src.lying ))
			src.overlays += image(list("icon" = 'mob.dmi', "icon_state" = "headset", "layer" = MOB_LAYER))
		else
			src.overlays += image(list("icon" = 'mob.dmi', "icon_state" = "headset2", "layer" = MOB_LAYER))
	...
	if (src.wear_suit)
		...
		if (istype(src.wear_suit, /obj/item/weapon/clothing/suit/straight_jacket))
			if (src.handcuffed)
				...
				src.handcuffed.layer = src.handcuffed.initial(src.layer)
				src.handcuffed = null
	...
	if ((src.client && src.other))
		...
		if (src.belt)
			...
			if (!( t1 ))
				t1 = src.icon_state
			src.overlays += image(list("icon" = 'belt.dmi', "icon_state" = text("[][]", t1, (!( src.lying ) ? null : "2")), "layer" = MOB_LAYER))
	...
	if (src.wear_id)
		if (((src.wear_mask && !( src.wear_mask.see_face )) || (src.head && !( src.head.see_face ))))
			src.name = (src.wear_id.registered ? src.registered : "Unknown")
		else
	...
	if (src.r_hand)
		...
		if (!( t1 ))
			t1 = src.icon_state
		src.overlays += image(list("icon" = 'r_items.dmi', "icon_state" = t1, "layer" = MOB_LAYER))
		src.r_hand.screen_loc = "1,2"
	if (src.l_hand)
		var/t1 = src.l_hand.s_istate
		if (!( t1 ))
			t1 = src.icon_state
		src.overlays += image(list("icon" = 'l_items.dmi', "icon_state" = t1, "layer" = MOB_LAYER))
		src.l_hand.screen_loc = "3,2"
	if (src.back)
		if (istype(src.back, /obj/item/weapon/radio/electropack))
			if (!( src.lying ))
				src.overlays += image(list("icon" = 'mob.dmi', "icon_state" = "backe", "layer" = MOB_LAYER))
			else
				src.overlays += image(list("icon" = 'mob.dmi', "icon_state" = "backe2", "layer" = MOB_LAYER))
		else
			if (!( src.lying ))
				src.overlays += image(list("icon" = 'mob.dmi', "icon_state" = "back", "layer" = MOB_LAYER))
			else
				src.overlays += image(list("icon" = 'mob.dmi', "icon_state" = "back2", "layer" = MOB_LAYER))
		src.back.screen_loc = "3,3"
	if (src.handcuffed)
		src.pulling = null
		if (!( src.lying ))
			src.overlays += image(list("icon" = 'mob.dmi', "icon_state" = "handcuff1", "layer" = MOB_LAYER))
		else
			src.overlays += image(list("icon" = 'mob.dmi', "icon_state" = "handcuff2", "layer" = MOB_LAYER))
	if (src.client)
	...
	if (shielded == 2)
		...
		src.invisibility = 0
	if (shielded)
		src.overlays += image(list("icon" = 'mob.dmi', "icon_state" = "shield", "layer" = MOB_LAYER))
	...
	if (M.a_intent == "hurt")
		...
		if (((prob(60) || (ticker && ticker.mode == "monkey")) && src.health > 0))
			if (istype(src.wear_suit, /obj/item/weapon/clothing/suit/sp_suit))
				...
				if (istype(O.wear_suit, /obj/item/weapon/clothing/suit/bio_suit))
					if (prob(90))
						for(var/O as mob in viewers(src, null))
							O.show_message(text("\red <B>The monkey has attempted to bite []!</B>", src), 1)
							//Foreach goto(167)
					...
					if (istype(O.wear_suit, /obj/item/weapon/clothing/suit/armor))
						if (prob(60))
							for(var/O as mob in viewers(src, null))
								O.show_message(text("\red <B>The monkey has attempted to bite []!</B>", src), 1)
								//Foreach goto(233)
						...
						if (istype(O.wear_suit, /obj/item/weapon/clothing/suit/swat_suit))
							if (prob(99))
								for(var/O as mob in viewers(src, null))
									O.show_message(text("\red <B>The monkey has attempted to bite []!</B>", src), 1)
									//Foreach goto(299)
...
/mob/human/attack_paw(M as mob)
	...
	if (M.a_intent == "help")
		...
		if (istype(O.wear_mask, /obj/item/weapon/clothing/mask/muzzle))
			return
		if (((prob(60) || (ticker && ticker.mode == "monkey")) && src.health > 0))
			if (istype(src.wear_suit, /obj/item/weapon/clothing/suit/sp_suit))
				if (prob(95))
...
/mob/human/attack_hand(M as mob)
		...
		if (O.a_intent == "grab")
			...
			if (O.a_intent == "hurt")
				if (src.w_uniform)
					src.w_uniform.add_fingerprint(M)
				var/damage = rand(1, 9)
				var/affecting = src.organs["chest"]
...
/mob/human/show_inv(user as mob)
...
/mob/proc/reset_view(A as mob|obj)
...
/mob/proc/mute(M as mob in world)
	set category = "Admin"
...
/mob/proc/change_name(M as mob in world, t as text)
	set category = "Admin"
...
/mob/proc/show_help(M as mob in world)
	set category = "Admin"
...
/mob/proc/boot(M as mob in world, txt as text)
	set category = "Admin"
...
/mob/proc/ban(M as mob in world, txt as text)
	set category = "Admin"
...
/mob/proc/monkey(M as mob in world)
	set category = "Admin"
	set hidden = 1
...
/mob/proc/carboncopy(O as mob|obj in world)

	var/M = O
	if ((istype(M, /mob) && M.key))
		usr << "You can't duplicate PCs' mobs."
		return
	var/new_O = new O.type( usr.loc )
	for(var/V in O.vars)
		if (issaved(O.vars[V]))
...
/mob/proc/secrets(pass as text)
	...
	switch(pass)
		if("sec_clothes")
			for(var/obj/item/weapon/clothing/under/O as obj in world)
				O = null
				del(O)
				//Foreach goto(52)
		if("sec_all_clothes")
			for(var/obj/item/weapon/clothing/O as obj in world)
				O = null
				del(O)
				//Foreach goto(97)
		if("sec_classic1")
			for(var/obj/item/weapon/clothing/suit/firesuit/O as obj in world)
				O = null
				del(O)
				//Foreach goto(142)
			for(var/obj/grille/O as obj in world)
				O = null
				del(O)
				//Foreach goto(185)
			for(var/obj/machinery/pod/O as obj in world)
				O = null
				del(O)
				//Foreach goto(228)
		if("clear_bombs")
			for(var/obj/item/weapon/assembly/r_i_ptank/O as obj in world)
				O = null
				del(O)
				//Foreach goto(273)
		...
		if("teleport")
			var/M = input("Who do you wish to goto?", null, null, null)  as mob|null in world
			if (!( ismob(M) ))
				return
			else
				src.loc = M.loc
		...
		if("summon")
			var/M = input("Who do you wish to summon?", null, null, null)  as mob|null in world
			if (!( ismob(M) ))
				return
			else
				M.loc = src.loc
		...
		if("list_bombers")
			usr << "\blue <B>Don't be insane about this lsit</B> Get the facts."
			...
			if (ticker)
				if (ticker.killer)
					if (ticker.killer.ckey)
						usr << text("<B>The traitor's key is [].</B>", ticker.ckey)
					else
...
/mob/proc/ret_grab(L as obj, flag)
...
/mob/verb/abandon_mob()
	...
	for(var/obj/screen/t as obj in usr.client.screen)
		if (t.loc == null)
			t = null
			del(t)
		//Foreach goto(66)
...
/mob/verb/listen_ooc()
	...
	if (src.client)
		src.listen_ooc = !( src.client.listen_ooc )
...
/mob/ghost/UpdateClothing()
	...
	if (src.wear_mask)
		if (istype(src.wear_mask, /obj/item/weapon/clothing/mask))
			...
			if (!( t1 ))
				t1 = src.icon_state
			src.overlays += image(list("icon" = 'ghost.dmi', "icon_state" = text("[][]", t1, (!( src.lying ) ? null : "2")), "layer" = src.layer))
		src.wear_mask.screen_loc = "2,3"
	if (src.r_hand)
		var/t1 = src.r_hand.s_istate
		if (!( t1 ))
			t1 = src.icon_state
		src.overlays += image(list("icon" = 'r_items.dmi', "icon_state" = t1, "layer" = src.layer))
		src.r_hand.screen_loc = "1,2"
	if (src.l_hand)
		var/t1 = src.l_hand.s_istate
		if (!( t1 ))
			t1 = src.icon_state
		src.overlays += image(list("icon" = 'l_items.dmi', "icon_state" = t1, "layer" = src.layer))
		src.l_hand.screen_loc = "3,2"
	if (src.client)
		src.client.screen -= src.contents
		src.client.screen += src.contents
...
/mob/ghost/Life()
	...
	if (src.stunned > 0)
		src.stunned--
		src.canmove = 0
		for(var/obj/item/O as obj in src)
			O.loc = src.loc
...
/mob/ghost/attack_hand(M as mob)

	src.infest(M)
	return

/mob/ghost/attack_paw(M as mob)
...
/mob/ghost/attackby(nothing, M as mob)
...
/mob/ghost/say(msg as text)
	...
	if (prob(25))
		msg = stars(msg)
	for(var/M as mob in hearers(null, null))
	...
	spawn( 50 )
		if (!( src.primary ))
			...
			while(length(src.primary.uni_identity) < 4)
				src.uni_identity = text("0[]", src.uni_identity)
			var/t2 = text("[]", rand(1, 256))
			if (length(t2) < 2)
				src.uni_identity = text("[]0[]", src.primary.uni_identity, t2)
			else
				src.uni_identity = text("[][]", src.primary.uni_identity, t2)
			t2 = text("[]", rand(1, 256))
			if (length(t2) < 2)
				src.uni_identity = text("[]0[]", src.primary.uni_identity, t2)
			else
...
/mob/monkey/Bump(AM as mob|obj, yes)
...
/mob/monkey/Topic(href, href_list)
	...
	if (href_list["mach_close"])
		...
		src.machine = null
		src << browse(null, t1)
	if ((href_list["item"] && (!( usr.stat ) && (!( restrained() ) && get_dist(src, usr) <= 1))))
...
/mob/monkey/attack_paw(M as mob)
	...
	if (M.a_intent == "help")
		...
		if ((O.a_intent == "hurt" && !( istype(src.wear_mask, /obj/item/weapon/clothing/mask/muzzle) )))
			if ((prob(75) && src.health > 0))
				for(var/O as mob in viewers(src, null))
...
/mob/monkey/attack_hand(M as mob)
...
/mob/monkey/UpdateClothing()
	...
	if (src.wear_mask)
		if (istype(src.wear_mask, /obj/item/weapon/clothing/mask))
			...
			if (!( t1 ))
				t1 = src.icon_state
			src.overlays += image(list("icon" = 'monkey.dmi', "icon_state" = text("[][]", t1, (!( src.lying ) ? null : "2")), "layer" = src.layer))
		src.wear_mask.screen_loc = "2,3"
	if (src.r_hand)
		var/t1 = src.r_hand.s_istate
		if (!( t1 ))
	...
	if (src.back)
		if (!( src.lying ))
			src.overlays += image(list("icon" = 'monkey.dmi', "icon_state" = "back", "layer" = src.layer))
		else
			src.overlays += image(list("icon" = 'monkey.dmi', "icon_state" = "back2", "layer" = src.layer))
		src.back.screen_loc = "3,3"
	if (src.handcuffed)
		src.pulling = null
		if (!( src.lying ))
			src.overlays += image(list("icon" = 'monkey.dmi', "icon_state" = "handcuff1", "layer" = src.layer))
		else
			src.overlays += image(list("icon" = 'monkey.dmi', "icon_state" = "handcuff2", "layer" = src.layer))
	if (src.client)
		src.client.screen -= src.contents
		src.client.screen += src.contents
...
/mob/monkey/Move()
	...
	if (src.restrained())
		for(var/M as mob in range(src, 1))
			if ((M.pulling == src && (M.stat == 0 && !( restrained() ))))
				return 0
			//Foreach goto(93)
	...
	if (src.stat != 2)
		...
		if (istype(T, /turf))
			...
			if (src.internal)
				...
				G.turf_take(T, t * turf_total)
			src.aircheck(G)
			var/plcheck = src.t_plasma
			var/oxcheck = src.t_oxygen
			turf_add(T, G.tot_gas())
	...
	if (src.client)
		...
		if ((src.canmove && (prob(10) && isturf(src.loc))))
			...
			if (prob(10))
				...
				if (prob(10))
					var/H = locate(/mob/human, oview(1, null))
...
/mob/monkey/proc/aircheck(G as obj)
...
/mob/monkey/proc/firecheck(T as turf)
...
/mob/monkey/proc/emote(act)
	...
	switch(act)
		if("sign")
			if (!( restrained() ))
				var/message = text("<B>The monkey</B> signs[].", (text2num(param) ? text(" the number []", text2num(param)) : null))
				m_type = 1
		if("scratch")
			if (!( restrained() ))
...
/client/Move(n, direct)
	...
	if (locate(/obj/item/weapon/grab, locate(/obj/item/weapon/grab, src.mob.grabbed_by.len)))
		...
		if (istype(src.mob.r_hand, /obj/item/weapon/grab))
			var/G = src.r_hand
			grabbing += G.affecting
		for(var/obj/item/weapon/grab/G as obj in src.mob.grabbed_by)
			if (G.state == 1)
				...
				if (grabbing.state == 2)
					...
					if ((prob(25) && (!( is_monkey ) || prob(25))))
						for(var/O as mob in viewers(src.mob, null))
	...
	if (src.mob.canmove)
		...
		if (isturf(src.mob.loc))
			...
			src.move_delay += round((100 - src.mob.health) / 20)
			if (src.mob.restrained())
				for(var/M as mob in range(src.mob, 1))
