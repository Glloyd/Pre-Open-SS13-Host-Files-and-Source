...
/obj/machinery/at_indicator/proc/update_icon()
	...
	if (SS13_airtunnel.operating == 1)
		...
		if (SS13_airtunnel.operating == 2)
			...
			var/C = pick(SS13_airtunnel.connectors)
			if (C.current == C)
				status = 0
			else
				if (!( C.current.next ))
...
/obj/machinery/computer/airtunnel/attack_hand(user as mob)
...
/obj/machinery/sec_lock/attack_hand(user as mob)
	...
	if ((usr.contents.Find(src) || (get_dist(src, usr) <= 1 && istype(src.loc, /turf))))
		...
		if (href_list["door1"])
			if (src.scan)
				...
				if ((L.Find(src.scan.assignment) || (src.scan.lab_access > 4 && src.access_level > 4)))
					if (src.d1.density)
						spawn( 0 )
							src.open()
							return
...
/obj/machinery/autolathe/attackby(O as obj, user as mob)
...
/obj/machinery/injector/attackby(W as obj, user as mob)

	var/ptank = W
	if (!( istype(ptank, /obj/item/weapon/tank) ))
		return
	var/T = get_step(src.loc, get_dir(user, src))
...
/datum/air_tunnel/air_tunnel1/New()
	...
	..()
	for(var/obj/move/airtunnel/A as obj in locate(/area/airtunnel1))
		A.master = src
		create()
		src.connectors += A
		//Foreach goto(21)
...
/datum/air_tunnel/proc/siphons()
	...
	switch(src.siphon_status)
		if(0.0)
			for(var/obj/machinery/atmoalter/siphs/S as obj in locate(/area/airtunnel1))
				S.t_status = 3
				//Foreach goto(42)
		if(1.0)
			for(var/obj/machinery/atmoalter/siphs/fullairsiphon/S as obj in locate(/area/airtunnel1))
				S.t_status = 2
				S.t_per = 1000000.0
				//Foreach goto(86)
			for(var/obj/machinery/atmoalter/siphs/scrubbers/S as obj in locate(/area/airtunnel1))
				S.t_status = 3
				//Foreach goto(136)
		if(2.0)
			for(var/obj/machinery/atmoalter/siphs/S as obj in locate(/area/airtunnel1))
...
/datum/air_tunnel/proc/extend()
	...
	while(src.operating == 2)
		var/ok = 1
		for(var/obj/move/airtunnel/connector/A as obj in src.connectors)
			if (!( A.current.next ))
				src.operating = 0
				return
			if (!( A.move_left() ))
