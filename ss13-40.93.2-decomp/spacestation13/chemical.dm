...
/obj/substance/chemical/proc/transfer_from(S as obj, amount)
	...
	if (amount >= s_volume)
		...
		var/U = S.split(amount)
		for(var/item in U.chemicals)
			var/C = U.chemicals[item]
			if (istype(C, /datum/chemical))
				var/N = src.chemicals[item]
				if (!( N ))
					N = new C.type( null )
					C.copy_data(N)
...
/obj/substance/chemical/proc/transfer_mob(M as mob, amount)

	if (!( ismob(M) ))
		return
	var/S = src.split(amount)
	for(var/item in S.chemicals)
		var/C = S.chemicals[item]
		if (istype(C, /datum/chemical))
			C.injected(M)
...
/obj/substance/gas/leak(T as turf)
	...
	src.turf_add(T, (src.co2 + src.oxygen) + src.plasma)
	return
...
/obj/substance/gas/proc/transfer_from(target as obj, amount)
...
/obj/substance/gas/proc/turf_add(target as turf, amount)
...
/obj/substance/gas/proc/turf_take(target as turf, amount)
...
/obj/substance/gas/proc/extract_toxs(target as turf)
...
/obj/substance/gas/proc/merge_into(target as obj)
...
/datum/chemical/proc/copy_data(C)
...
/datum/chemical/pl_coag/injected(M as mob, zone)
...
/datum/chemical/l_plas/injected(M as mob, zone)
	...
	switch(zone)
		if("eye")
			...
			M.plasma += volume * 6
			for(var/obj/item/weapon/implant/tracking/T as obj in M)
				M.plasma += 1
				T = null
				del(T)
				//Foreach goto(133)
...
/datum/chemical/s_tox/injected(M as mob, zone)
...
/datum/chemical/epil/injected(M as mob, zone)
...
/datum/chemical/ch_cou/injected(M as mob, zone)
...
/datum/chemical/rejuv/injected(M as mob, zone)
