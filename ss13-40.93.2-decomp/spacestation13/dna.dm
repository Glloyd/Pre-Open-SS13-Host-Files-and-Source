...
/obj/machinery/computer/dna/attack_paw(user as mob)

	return src.attack_hand(user)
	return

/obj/machinery/computer/dna/attack_hand(user as mob)
		...
		if (href_list["execute"])
			if ((src.scan && src.scan.function))
				switch(src.scan.function)
					if("data_mutate")
						if (src.modify)
							if (!( findtext(src.scan.data, "-", 1, null) ))
								...
								var/d = findtext(src.data, "-", 1, null)
								var/t = copytext(src.data, d + 1, length(src.data) + 1)
								d = text2num(copytext(1, d, null))
								if ((d && (t && (src.modify.data && (src.scan.data && length(src.modify.data) >= ((length(t) + d) - 1))))))
									src.modify.data = text("[][][]", copytext(src.modify.data, 1, d), t, (length(src.data) > (length(t) + d) ? copytext(src.data, length(t) + d, length(src.data) + 1) : null))
								else
									src.temp = "Disk Failure: Cannot examine data! (Null or wrong format)"
						else
					...
					if("data_scramble")
						if (src.modify)
							if (length(text("[]", src.modify.data)) >= 1)
								src.modify.data = scram(length(text("[]", src.data)))
					...
					if("disk_dis")
						if (src.modify)
							src.temp = text("Function: [][]<BR>Data: []", src.modify.function, (src.special ? text("-[]", src.special) : null), src.modify.data)
						else
							src.temp = "Disk Failure: Cannot read target disk!"
					if("disk_erase")
						if (src.modify)
					...
					if("data_trun")
						if (src.modify)
							if ((src.modify.data && src.scan.data))
								...
								switch(copytext(src.data, 1, 2))
									if("a")
										if (l1 > l2)
											src.data = copytext(src.modify.data, 1, (l1 - l2) + 1)
										else
											src.modify.data = ""
										src.temp = text("Done!<BR>New Data:<BR>[]", src.modify.data)
									if("b")
										if (l1 > l2)
											src.data = copytext(src.modify.data, l2, l1 + 1)
										else
											src.modify.data = ""
										src.temp = text("Done!<BR>New Data:<BR>[]", src.modify.data)
									if("c")
										if (l1 >= l2)
											src.data = copytext(src.modify.data, l1 - l2, l1 + 1)
										src.temp = text("Done!<BR>New Data:<BR>[]", src.modify.data)
									else
										if (l1 >= l2)
											src.data = copytext(src.modify.data, 1, l2 + 1)
										src.temp = text("Done!<BR>New Data:<BR>[]", src.modify.data)
							else
...
/obj/machinery/dna_scanner/relaymove(user as mob)
...
/obj/machinery/dna_scanner/attackby(G as obj, user as mob)
	...
	if (src.status == "load")
		...
		if (src.status == "dna_trun")
			if (istype(M, /mob))
				...
				M.temp = "Process terminated due to lack of occupant in DNA chamber."
				M.status = null
		else
			if (M.status == "dna_replace")
				if (istype(M, /mob))
					var/t = null
					switch(M.prog_p1)
						if("UI")
				...
				if (M.status == "dna_add")
					...
					M.status = null
					M.temp = "Unknown system error."
	for(var/O as mob in viewers(1, src))
...
/obj/machinery/scan_console/attack_hand(user as mob)
	...
	if (src.temp)
		var/dat = text("[]<BR><BR><A href='?src=\ref[];clear=1'>Clear Message</A>", src.temp, src)
	else
		if (src.connected)
			...
			dat += text("Disk: <A href='?src=\ref[];scan=1'>[]</A><BR>\n[]<BR>\n[]<BR>", src, (src.scan ? text("[]", src.scan.name) : "----------"), (src.scan ? text("<A href='?src=\ref[];u_dat=1'>Upload Data</A>", src) : "No disk to upload"), ((src.data || (src.func || src.special)) ? text("<A href='?src=\ref[];c_dat=1'>Clear Data</A><BR><A href='?src=\ref[];e_dat=1'>Execute Data</A><BR>Function Type: [][]<BR>Data: []", src, src, src.func, (src.special ? text("-[]", src.special) : null), src.data) : "No data uploaded"))
...
/obj/machinery/scan_console/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || (get_dist(src, usr) <= 1 && istype(src.loc, /turf))))
		...
		if (href_list["locked"])
			if ((src.connected && src.connected.occupant))
				src.locked = !( src.connected.locked )
		if (href_list["scan"])
			if (src.scan)
				src.scan.loc = src.loc
				src.scan = null
		...
		if (href_list["u_dat"])
			if ((src.scan && !( src.status )))
				if ((src.scan.function && src.function != "storage"))
	...
	if ((src.occupant && src.occupant.primary))
		switch(src.occupant.primary.spec_identity)
			if("5BDFE293BA5500F9FFFD500AAFFE")
				if (!( istype(src.occupant, /mob/human) ))
					for(var/obj/O as obj in src.occupant)
						O = null
						del(O)
						//Foreach goto(78)
		...
		if (istype(src.occupant, /mob/human))
			...
			if (reg_dna[text("[]", H.primary.uni_identity)])
				H.rname = reg_dna[text("[]", H.uni_identity)]
			else
				if (findname("Unknown"))
					var/counter = 1
			...
			var/speak = (length(H.primary.struc_enzyme) >= 25 ? hex2num(copytext(H.struc_enzyme, 22, 25)) : 9999)
			var/ears = (length(H.primary.struc_enzyme) >= 10 ? hex2num(copytext(H.struc_enzyme, 7, 10)) : 9999)
			var/vision = (length(H.primary.struc_enzyme) >= 16 ? hex2num(copytext(H.struc_enzyme, 13, 16)) : 1)
			var/mental1 = (length(H.primary.struc_enzyme) >= 31 ? hex2num(copytext(H.struc_enzyme, 28, 31)) : 1)
			var/mental2 = (length(H.primary.struc_enzyme) >= 28 ? hex2num(copytext(H.struc_enzyme, 25, 28)) : 1)
			var/speak2 = (length(H.primary.struc_enzyme) >= 22 ? hex2num(copytext(H.struc_enzyme, 19, 22)) : 1)
			H.sdisabilities = 0
			H.disabilities = 0
			if (speak < 3776)
			...
			if (length(H.primary.uni_identity) >= 20)
				t1 = copytext(H.uni_identity, 19, 21)
				if (hex2num(t1) > 127)
					H.gender = "female"
				else
					H.gender = "male"
			...
			if (length(H.primary.uni_identity) >= 18)
				t1 = copytext(H.uni_identity, 17, 19)
...
/obj/machinery/restruct/relaymove(user as mob)
...
/obj/machinery/restruct/attackby(G as obj, user as mob)
