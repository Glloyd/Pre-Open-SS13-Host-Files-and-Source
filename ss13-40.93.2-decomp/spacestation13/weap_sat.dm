...
/obj/machinery/computer/teleporter/proc/find_loc(R as obj)
...
/obj/machinery/teleport/hub/proc/teleport(M as mob|obj)

	var/l = src.loc
	var/com = locate(/obj/machinery/computer/teleporter, locate(l.x - 2, l.y, l.z))
	if (!( com ))
		return
	var/target = com.locked
	if (!( com.locked ))
		for(var/O as mob in hearers(src, null))
	...
	if (istype(M, /atom/movable))
		...
		for(var/B as mob in hearers(src, null))
...
/obj/machinery/teleport/station/verb/engage()
	...
	var/l = src.loc
	var/com = locate(/obj/machinery/teleport/hub, locate(l.x + 1, l.y, l.z))
	if (com)
...
/obj/effects/smoke/proc/Life()
	...
	if (src.amount <= 0)
		...
		del(src)
		return
	var/T = get_step(src, turn(src.dir, pick(90, 0, 0, -90.0)))
	if ((T && T.density))
