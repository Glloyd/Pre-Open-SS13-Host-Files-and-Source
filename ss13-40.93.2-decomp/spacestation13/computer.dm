...
/obj/machinery/computer/security/attack_paw(user as mob)

	return src.attack_hand(user)
	return

/obj/machinery/computer/security/attack_hand(user as mob)

	var/list/L = list(  )
	user.machine = src
	for(var/obj/machinery/camera/C as obj in world)
...
/obj/machinery/computer/security/check_eye(user as mob)
...
/obj/machinery/computer/meteorhit(O as obj)
...
/obj/machinery/computer/card/attack_paw(user as mob)

	return src.attack_hand(user)
	return

/obj/machinery/computer/card/attack_hand(user as mob)
			...
			if (istype(user, /mob/human))
				...
				for(var/t in L)
					...
					if (counter >= 4)
						...
						counter = 1
					//Foreach goto(912)
				d1 = text("[] :<BR>\nGeneral Access Level: []<BR>\nLaboratory Access: []<BR>\nReactor/Engine Access: []<BR>\nMain Systems Access: []<BR>\nRegistered: <A href='?src=\ref[];reg=1'>[]</A><BR>\nAssignment: []<BR>\n[]<BR>", src.modify.name, vo, vl, ve, va, src, (src.registered ? text("[]", src.registered) : "{None: Click to modify}"), (src.modify.assignment ? text("[]", src.assignment) : "None"), assign)
			else
				var/counter = 1
				for(var/t in L)
					...
					if (counter >= 4)
						...
						counter = 1
					//Foreach goto(1057)
				d1 = text("[] :<BR>\n[] []<BR>\n[] []<BR>\n[] []<BR>\n[] []<BR>\n[] <A href='?src=\ref[];reg=1'>[]</A><BR>\n[] []<BR>\n[]<BR>", stars("modify.name"), stars("General Access Level:"), vo, stars("Laboratory Access:"), vl, stars("Reactor/Engine Access:"), ve, stars("Main Systems Access:"), va, stars("Registered:"), src, (src.modify.registered ? text("[]", stars(src.registered)) : text("[]", stars("{None: Click to modify}"))), stars("Assignment:"), (src.modify.assignment ? text("[]", stars(src.assignment)) : "None"), assign)
		if (istype(user, /mob/human))
...
/obj/machinery/computer/card/Topic(href, href_list)
	...
	if ((get_dist(src, usr) <= 1 && istype(src.loc, /turf)))
		...
		if (href_list["modify"])
			if (src.modify)
				src.name = text("[]'s ID Card ([]>[]-[]-[])", src.modify.registered, src.access_level, src.lab_access, src.engine_access, src.air_access)
		...
		if (href_list["auth"])
			if ((!( src.authenticated ) && (src.scan && (src.modify || src.mode))))
				if ((src.scan.assignment == "Captain" || src.assignment == "Head of Personnel"))
					src.authenticated = 1
		if (href_list["vo"])
...
/obj/machinery/computer/pod/proc/alarm()
	...
	if (!( src.connected ))
		...
		return
	for(var/obj/machinery/door/poddoor/M as obj in world)
		if (M.id == src.id)
			spawn( 0 )
				M.openpod()
				return
...
/obj/machinery/computer/pod/New()
	...
	..()
	spawn( 5 )
		for(var/obj/machinery/mass_driver/M as obj in world)
			if (M.id == src.id)
				src.connected = M
			else
...
/obj/machinery/computer/pod/attack_paw(user as mob)

	return src.attack_hand(user)
	return

/obj/machinery/computer/pod/attack_hand(user as mob)

	var/dat = "<HTML><BODY><TT><B>Mass Driver Controls</B>"
	user.machine = src
	if (src.timing)
		var/d2 = text("<A href='?src=\ref[];time=0'>Stop Time Launch</A>", src)
	else
...
/obj/machinery/pod/meteorhit(O as obj)
...
/obj/machinery/pod/relaymove(user as mob, direction)
...
/obj/machinery/pod/verb/unload(A as mob|obj in src)
	set src in oview(1)
...
/obj/machinery/recon/meteorhit(O as obj)
...
/obj/machinery/recon/verb/unload(A as mob|obj in src)
	set src in oview(1)
	...
	if (istype(A, /atom/movable))
		A.loc = src.loc
		for(var/O as mob in view(src, null))
			if ((O.client && !( O.blinded )))
				O << text("\blue <B> [] unloads [] from []!</B>", usr, A, src)
			//Foreach goto(53)
		if (ismob(A))
...
/obj/machinery/recon/relaymove(user as mob, direction)
...
/turf/space/attack_paw(user as mob)

	return src.attack_hand(user)
	return

/turf/space/attack_hand(user as mob)
...
/turf/space/attackby(T as obj, user as mob)
...
/turf/space/Entered(A as mob|obj)
