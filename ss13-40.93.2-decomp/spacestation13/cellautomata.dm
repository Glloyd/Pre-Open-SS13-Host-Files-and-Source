...
/obj/team/proc/process()
	...
	if (src.base)
		var/starting = locate(text("landmark*CTF-base-[]", src.base))
		while(locate(text("landmark*CTF-supply-[]", src.base)))
			var/L = locate(text("landmark*CTF-supply-[]", src.base))
		...
		while(locate(text("landmark*CTF-wardrobe-[]", src.base)))
			var/L = locate(text("landmark*CTF-wardrobe-[]", src.base))
			switch(src.color)
				if("blue")
			...
			del(L)
		if (starting)
			for(var/mob/human/H as mob in src.members)
				H.loc = starting.loc
...
/obj/team/Topic(href, href_list)
	...
	if (usr.CanAdmin())
		...
		if (href_list["captain"])
			var/list/L = list(  )
			for(var/H as mob in world)
				if (H.client)
					L += H
				//Foreach goto(331)
			for(var/obj/team/T as obj in world)
				L -= T.members
				L -= T.captain
				//Foreach goto(370)
			var/m = input(usr, "Please select a new captain", null, null)  as null|anything in L
			if (ismob(m))
		...
		src.show_screen(usr)
	for(var/mob/human/H as mob in world)
...
/obj/ctf_assist/New()
	...
	for(var/obj/begin/B as obj in world)
	...
	for(var/mob/human/M as mob in world)
		...
		if (M.start)
			M.primary = null
			del(M.primary)
			for(var/obj/item/weapon/I as obj in M)
				M = null
				del(M)
				//Foreach goto(165)
			M.start = 0
...
/obj/ctf_assist/proc/next_pick()
	...
	if (src.pickers_left.len < 1)
		for(var/obj/team/T as obj in src)
...
/obj/ctf_assist/proc/show_pick(user as mob)
	...
	for(var/mob/human/H as mob in src.players_left)
...
/obj/ctf_assist/proc/check_win(O as obj)
	...
	if (src.wintype == "none")
		return
	var/F = locate(/obj/item/weapon/paper/flag)
	var/winning = 1
	for(var/obj/item/weapon/paper/flag/L as obj in world)
	...
	if (winner)
		...
		for(var/mob/human/H as mob in winner.members)
			if (H.client)
...
/obj/ctf_assist/Topic(href, href_list)
	...
	if (href_list["start"])
		...
		for(var/mob/human/H as mob in world)
			...
			//Foreach goto(923)
		world << "<B>STARTING!!!</B>"
		for(var/obj/landmark/alterations/A as obj in world)
			switch(A.name)
				if("prison shuttle")
			...
			//Foreach goto(1035)
		for(var/obj/closet/wardrobe/W as obj in world)
			W = null
			del(W)
			//Foreach goto(1238)
		for(var/obj/item/weapon/clothing/under/T as obj in world)
			T = null
			del(T)
			//Foreach goto(1281)
		if (src.ejectengine)
			for(var/obj/machinery/computer/engine/T as obj in world)
				T = null
				del(T)
				//Foreach goto(1333)
		for(var/obj/landmark/alterations/A as obj in world)
		...
		for(var/obj/team/T as obj in src)
			...
			//Foreach goto(1545)
		if (src.paint_cans)
			for(var/obj/secloset/highsec/S as obj in world)
		...
		if (src.neutral_replace)
			while(src.avail_bases.len > 0)
				...
				src.avail_bases -= t
				var/L = locate(text("landmark*CTF-supply-[]", t))
				var/obj/item/weapon/paper/flag/F = new /obj/item/weapon/paper/flag( L.loc )
				F.name = "flag- 'NEUTRAL Team's Flag'"
				F.icon_state = "flag_neutral"
		...
		for(var/obj/begin/B as obj in world)
			if (locate(/obj/grille, B.loc))
				for(var/obj/grille/G as obj in B.loc)
					G = null
					del(G)
					//Foreach goto(1789)
			//Foreach goto(1742)
...
/obj/admins/Topic(href, href_list)
	...
	if (href_list["p_send2"])
		if (locate(href_list["p_send2"]))
			var/M = locate(href_list["p_send2"])
			if (!( ismob(M) ))
				return
			...
			if (!( t ))
				return
			if (M.client.holder)
				M << text("\blue Admin PM from-<B><A href='?src=\ref[];p_send2=\ref[]'>[]</A></B>: []", M.holder, usr, usr.key, t)
			else
	...
	if (href_list["secrets2"])
		if ((src.rank in list( "Game Master", "Administrator", "Major Administrator", "Primary Administrator" )))
			...
			switch(href_list["secrets2"])
				...
				if("toxic")
					for(var/obj/machinery/atmoalter/siphs/fullairsiphon/O as obj in world)
						O.t_status = 3
						//Foreach goto(3194)
					for(var/obj/machinery/atmoalter/siphs/scrubbers/O as obj in world)
						O.t_status = 1
						O.t_per = 1000000.0
						//Foreach goto(3234)
					for(var/obj/machinery/atmoalter/canister/O as obj in world)
					...
					if (ticker)
						if (ticker.killer)
							if (ticker.killer.ckey)
								alert(text("<B>The traitor's key is [].</B>", ticker.ckey), null, null, null, null, null)
							else
...
/world/New()
	...
	for(var/obj/machinery/nuclearbomb/N as obj in world)
		if (N.r_code == "ADMIN")
			N.r_code = nuke_code
		//Foreach goto(260)
	for(var/mob/human/H as mob in world)
...
/atom/movable/Bump(A as mob|obj|turf|area, yes)
...
/atom/verb/point()
	...
	if ((!( usr ) || !( isturf(usr.loc) )))
		return
	if ((usr.stat == 0 && !( restrained() )))
		var/obj/point/P = new /obj/point( (isturf(src) ? src : src.loc) )
		spawn( 20 )
			P = null
			del(P)
			return
		for(var/M as mob in viewers(usr, null))
...
/datum/control/gameticker/proc/check_win()
	...
	switch(src.mode)
			...
			switch(src.objective)
				if("alone")
					for(var/M as mob in world)
						...
						//Foreach goto(79)
				if("assassinate")
					if ((M.target && src.target.stat != 2))
						traitorwin = 0
				if("theft")
					var/list/L = list(  )
					if (src.killer)
						L += src.killer.contents
						for(var/obj/item/weapon/storage/S as obj in src.killer)
							L += S.return_inv()
							//Foreach goto(228)
						for(var/obj/item/weapon/gift/G as obj in src)
							L += G.gift
							//Foreach goto(271)
					traitorwin = 0
					switch(src.theft_obj)
						if("lasergun")
							for(var/obj/item/weapon/gun/energy/laser_gun/O as obj in L)
								if (O.charges == 25)
									traitorwin = 1
								else
						...
						if("plasmabomb")
							for(var/obj/item/weapon/assembly/r_i_ptank/O as obj in L)
								var/P = O.part3
								if ((P.gas.plasma >= 1600000.0 && P.temperature >= 500))
									traitorwin = 1
								else
									//Foreach continue //goto(413)
							item = "a fully armed and heated plasma bomb"
						if("flashbang")
							for(var/obj/item/weapon/flashbang/O as obj in L)
								traitorwin = 1
								else
								//Foreach goto(500)
							item = "a flashbang"
						if("captaincard")
							for(var/obj/item/weapon/card/id/O as obj in L)
						...
						if("sleepingpills")
							for(var/obj/item/weapon/pill_canister/O as obj in L)
								for(var/obj/item/weapon/m_pill/sleep/P as obj in O)
									if (P.amount == 30)
										traitorwin = 1
									else
						...
						if("pl_flask")
							for(var/obj/item/weapon/flasks/O as obj in L)
								if (O.plasma == 500)
									traitorwin = 1
								else
		...
		if("monkey")
			...
			if (monkeywin)
				monkeywin = 0
				for(var/mob/monkey/M as mob in world)
					if (M.stat != 2)
						var/T = M.loc
						if (istype(T, /turf))
							if ((T in A))
			...
			if (monkeywin)
				...
				for(var/mob/human/M as mob in world)
					...
					//Foreach goto(1254)
		if("nuclear")
			if (M.objective != "Success")
				var/disk_on_shuttle = 0
				for(var/obj/item/weapon/disk/nuclear/N as obj in world)
					if (N.loc)
						var/T = get_turf(N)
						if ((T in A))
							disk_on_shuttle = 1
...
/datum/control/gameticker/process()
		...
		if("nuclear")
			...
			for(var/mob/human/M as mob in world)
				if ((M.client && M.start))
					mobs += M
				//Foreach goto(260)
			var/O = locate("landmark*CTF-rogue")
			if (mobs.len >= 4)
				...
				while(amount > 0)
					...
					if (istype(H, /mob/human))
						...
						G.layer = 20
						H.belt = G
						var/obj/item/weapon/radio/headset/R = new /obj/item/weapon/radio/headset( H )
						R.freq = 146.5
						R.layer = 20
						H.w_radio = R
		if("meteor")
	...
	DivideOccupations()
	for(var/obj/manifest/M as obj in world)
		M.manifest()
		//Foreach goto(837)
	for(var/mob/human/H as mob in world)
		if (H.start)
	...
	switch(src.mode)
		if("traitor")
			...
			var/backup = mobs
			spawn( 100 )
				var/obj/item/weapon/syndicate_uplink/traitor_item = new /obj/item/weapon/syndicate_uplink( src.killer.loc )
			...
			spawn( rand(600, 1800) )
				...
				for(var/obj/machinery/computer/communications/C as obj in world)
		...
		if("nuclear")
			spawn( 50 )
				var/L = locate("landmark*Nuclear-Disk")
	...
	for(var/obj/start/S as obj in world)
		S = null
		del(S)
		//Foreach goto(2445)
	return
...
/datum/control/cellular/process()
	...
	while(!( ticker ))
		for(var/M as mob in world)
			...
			//Foreach goto(28)
		sleep(10)
	for(var/turf/station/T as turf in world)
		if (T.updatecell)
			updatecell()
		//Foreach goto(73)
	sleep(3)
	for(var/M as mob in world)
		...
		//Foreach goto(126)
	sleep(3)
	for(var/obj/move/S as obj in world)
		S.process()
		//Foreach goto(167)
	sleep(2)
	for(var/obj/machinery/M as obj in world)
		M.process()
		//Foreach goto(213)
