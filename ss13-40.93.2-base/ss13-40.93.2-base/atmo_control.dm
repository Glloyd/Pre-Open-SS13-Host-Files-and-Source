...
/obj/machinery/meter/Click()
	...
	if (get_dist(usr, src) <= 3)
		if (src.target)
			usr << text("\green <B>[] / []</B>", src.target.gas.tot_gas(), src.target.gas.maximum)
		else
...
/obj/machinery/connector/receive_gas(var/obj/substance/gas/t_gas as obj)

	var/obj/machinery/atmoalter/canister/C = locate(/obj/machinery/atmoalter/canister, src.loc)
	if (C.destroyed)
		var/turf/T = src.loc
		if (!( istype(T, /turf) ))
			return
		...
		return
	var/amount = C.gas.maximum - (C.gas.oxygen + C.gas.co2 + C.gas.plasma)
...
/obj/machinery/pipes/receive_gas(var/obj/substance/gas/t_gas as obj, from as obj)

	if (t_gas.tot_gas() > src.gas.maximum)
		src.gas.merge_into(t_gas)
		src.explode()
		return
	spawn( 1 )
		if (from == src.node1)
